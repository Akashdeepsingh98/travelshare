...
2. Security
   ...
   - Ensure only guide owners can modify their guides

3. Changes
   - Add foreign key relationships between tables
   - Create indexes for performance
   - Add triggers for updating timestamps
   - Add helper functions for guide operations
*/
...
DROP FUNCTION IF EXISTS update_travel_guides_updated_at();
...
-- Drop existing function if it exists
DROP FUNCTION IF EXISTS get_guide_content(uuid);
...
DROP FUNCTION IF EXISTS validate_post_exists();
DROP FUNCTION IF EXISTS validate_itinerary_exists();
