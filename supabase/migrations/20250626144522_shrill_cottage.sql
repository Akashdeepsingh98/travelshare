/*
  ...
  2. Security
    ...
    - Add policies for public/private communities
    - Add policies for community membership
    - Add policies for sharing posts
...
-- Fixed policy to avoid using OLD reference
...
CREATE POLICY "Community admins can remove shared posts"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM community_members 
      WHERE community_id = community_shared_posts.community_id 
      AND user_id = auth.uid() 
      AND role = 'admin'
    )
  );
