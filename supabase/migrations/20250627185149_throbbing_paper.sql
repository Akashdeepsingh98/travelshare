/*
  # Itineraries and Itinerary Items Tables

  1. New Tables
    - `itineraries` - Stores travel itineraries created by users
    - `itinerary_items` - Stores individual items within an itinerary
...
-- Itineraries policies - check if they exist first
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Itinerary items policies - check if they exist first
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
...
-- Function to update updated_at timestamp - check if it exists first
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tgname = 'update_itineraries_updated_at'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'update_itinerary_items_updated_at'
  ) THEN
