/*
  # Travel Guides and Content Items Schema

  1. New Tables
    - `travel_guides` - Stores travel guides created by users
  ...
  3. Functions
    - `update_travel_guides_updated_at()` - Updates timestamp on guide changes
    - `get_guide_content()` - Retrieves guide content with related data
    - Validation functions for content references
*/
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'update_travel_guides_updated_at') THEN
    CREATE FUNCTION update_travel_guides_updated_at()
    RETURNS TRIGGER AS $$
    BEGIN
      NEW.updated_at = now();
      RETURN NEW;
