...
1. New Tables
   ...
   - `group_members`: Tracks membership and roles in group chats
   - `group_messages`: Stores messages sent in group chats

2. Security
   - Enable RLS on all tables
   - Add policies for proper access control
   - Ensure only group members can view/send messages

3. Changes
   - Add foreign key relationship between tables
   - Create indexes for performance
   - Add triggers for updating timestamps
   - Add helper functions for group operations
*/
...
DROP POLICY IF EXISTS "Group creator can delete group chats" ON group_chats;
...
DROP POLICY IF EXISTS "Group members can leave group" ON group_members;
...
DROP POLICY IF EXISTS "Message sender can update own message" ON group_messages;
DROP POLICY IF EXISTS "Message sender or group admin can delete messages" ON group_messages;
...
DROP FUNCTION IF EXISTS update_group_chat_activity();
...
DROP TRIGGER IF EXISTS update_group_chat_activity ON group_messages;
...
-- Drop existing function if it exists
DROP FUNCTION IF EXISTS create_group_chat(text, text, uuid, uuid[]);
...
-- Drop existing function if it exists
DROP FUNCTION IF EXISTS get_unread_group_messages_count(uuid);
