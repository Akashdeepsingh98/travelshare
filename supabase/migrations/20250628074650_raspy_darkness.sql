...
-- RLS Policies for travel_guides - Check if they exist first
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tgname = 'update_travel_guides_updated_at'
  ) THEN
...
-- Create function to get guide content with details if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_proc 
    WHERE proname = 'get_guide_content'
  ) THEN
...
-- Create functions to validate content items if they don't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_proc 
    WHERE proname = 'validate_post_exists'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_proc 
    WHERE proname = 'validate_itinerary_exists'
  ) THEN
...
-- Create triggers to validate content items if they don't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'validate_post_reference'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'validate_itinerary_reference'
  ) THEN
